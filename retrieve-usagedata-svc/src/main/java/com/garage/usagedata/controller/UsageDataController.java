...
import com.garage.usagedata.remote.bean.Data_DataTotDtlRetvProdNm;
import com.garage.usagedata.remote.bean.Data_DrctlyUseQntMyOllehListRetv;
...
public class UsageDataController {
	...
	@GetMapping("/retrieveBillPreTmonUseQntMyOlleh/{svcContId}/{retvStDt}/{sbscrNo}")
	public ResponseEntity<Data_DrctlyUseQntMyOllehListRetv> getRetrieveBillPreTmonUseQntMyOlleh(@PathVariable("svcContId") String svcContId, @PathVariable("retvStDt") String retvStDt,@PathVariable("sbscrNo") String sbscrNo) {
		
		log.debug("Start [retrieveBillPreTmonUseQntMyOlleh] service...");
		
		String url = "http://" + targetServiceName + "/usage/retrieveBillPreTmonUseQntMyOlleh/" + svcContId + "/" + retvStDt + "/" + sbscrNo;
		
		log.debug("url:" + url);
		
		ResponseEntity<Data_DrctlyUseQntMyOllehListRetv> response = restTemplate.exchange(
	            url,
	            HttpMethod.GET,
	            null,
	            new ParameterizedTypeReference<Data_DrctlyUseQntMyOllehListRetv>(){});
		
		Optional<Data_DrctlyUseQntMyOllehListRetv> dbUsageData = Optional.ofNullable(response.getBody());
		if ( dbUsageData.isPresent() ) {
			log.debug("Return usage data from DB");
			log.debug("data:" + dbUsageData.get().toString());
			return new ResponseEntity<>(dbUsageData.get(), HttpStatus.OK);
		} else {
			log.debug("No data found");
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
	...
	@GetMapping("/dataTotDtlRetvProdNm/{svcContId}/{rqtDate}/{mdn}")
	public ResponseEntity<List<Data_DataTotDtlRetvProdNm>> getDataTotDtlRetvProdNm(@PathVariable("svcContId") String svcContId, @PathVariable("rqtDate") String rqtDate, @PathVariable("mdn") String mdn) {
		
		log.debug("Start [dataTotDtlRetvProdNm] service...");
		
		String url = "http://" + targetServiceName + "/usage/dataTotDtlRetvProdNm/" + svcContId + "/" + rqtDate + "/" + mdn;
		
		log.debug("url:" + url);
		
		ResponseEntity<List<Data_DataTotDtlRetvProdNm>> response = null;
		try {
			response = restTemplate.exchange(
		            url,
		            HttpMethod.GET,
		            null,
		            new ParameterizedTypeReference<List<Data_DataTotDtlRetvProdNm>>(){});
		...
		Optional<List<Data_DataTotDtlRetvProdNm>> dbUsageData = Optional.ofNullable(response.getBody());
		if ( dbUsageData.isPresent() ) {
			log.debug("Return usage data from DB");
			log.debug("data:" + dbUsageData.get().toString());
			return new ResponseEntity<>(dbUsageData.get(), HttpStatus.OK);
