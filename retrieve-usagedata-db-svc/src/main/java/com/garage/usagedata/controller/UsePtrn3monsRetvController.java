...
public class UsePtrn3monsRetvController {
	...
	@GetMapping("{svcContId}")
	public ResponseEntity<List<Data_UsePtrn3monsRetv>> getDataAll(@PathVariable("svcContId") String svcContId) {
		
		log.debug("Get Data with ID = " + svcContId );
		Optional<List<Data_UsePtrn3monsRetv>> data = repository.findBySvcContId(svcContId);
		
		if ( data.isPresent() ) {
			
			data.get().forEach(s -> {
				String redisKey = "usePtrn3monsRetv" + "-" + svcContId + "-" + s.getRetvYm();
				
				// 결과를 Cache Manager로 전달. cache 반영 처리 위임.
				makeCacheData(redisKey, s);
			});
